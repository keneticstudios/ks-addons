...
local function getDropModels(inv)
	local models = {}
	local count = 0
	if not inv or not inv.items then return {'prop_paper_bag_small'} end
    
	-- Минаваме през всички слотове на дропа
	for slot, v in pairs(inv.items) do
		if count >= 10 then break end
		local itemDef = Items(v.name)
		if itemDef then
			local model = itemDef.model or (itemDef.client and itemDef.client.model)
			if model then
	...
	if #models == 0 then
		models = {'prop_paper_bag_small'}
	end
	return models
end

-- 2. Функция за синхронизация (добавете я под getDropModels)
local function SyncDropVisuals(invId)
    local inv = Inventories[invId]
    if inv and inv.type == 'drop' then
        local models = getDropModels(inv)
        if Inventory.Drops[invId] then
            Inventory.Drops[invId].models = models
        end
        TriggerClientEvent('ks-addons:drop:updateDropModels', -1, invId, models)
    end
end
...
function Inventory.Remove(inv)
	...
	if inv then
		if inv.type == 'drop' then
			TriggerClientEvent('ks-addons:drop:removeDrop', -1, inv.id)
...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	TriggerClientEvent('ks-addons:drop:createDrop', -1, dropId, Inventory.Drops[dropId])
...
local function dropItem(source, playerInventory, fromData, data)
	...
	local models = getDropModels(inventory)
	Inventory.Drops[dropId] = {
		coords = inventory.coords, 
	...
	TriggerClientEvent('ks-addons:drop:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
			...
			if fromInventory.type == 'drop' then SyncDropVisuals(fromInventory.id) end
            if toInventory.type == 'drop' then SyncDropVisuals(toInventory.id) end
            CreateThread(function()
