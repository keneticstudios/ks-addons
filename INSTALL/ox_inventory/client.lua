...
local function canOpenInventory()
    ...
    if IsPauseMenuActive() and not invOpen then return end
...
function client.openInventory(inv, data)
	...
	TriggerEvent('ks-addons:pedpreview:openFrontendPed')
...
function client.closeInventory(server)
	...
	if invOpen then
		TriggerEvent('ks-addons:pedpreview:closeFrontendPed')
		invOpen = nil
...
local function createDrop(dropId, data)
	...
	point.onExit = function() end 
	client.drops[dropId] = point
end


RegisterNetEvent('ks-addons:drop:createDrop', function(dropId, data, owner, slot)
    if client.drops[dropId] then client.drops[dropId]:remove() end
...
RegisterNetEvent('ks-addons:drop:removeDrop', function(dropId)
    if client.drops[dropId] then
        client.drops[dropId]:remove()
...
-- Clothing slot to component/prop mapping
-- Based on Config.ClothingItems

-- Store original clothing state for toggle functionality
