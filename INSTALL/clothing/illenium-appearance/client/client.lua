...
-- [NEW] Променливи за ks-addons логиката
local changedClothes = {}

-- [NEW] Дефиниция за "съблечен" (записва се в базата, когато купуваш дрехи)
local strippedDefaults = {
    components = {
        [1] = { drawable = 0, texture = 0 },   -- Mask
        [3] = { drawable = 15, texture = 0 },  -- Arms (Gloves)
        [4] = { drawable = 21, texture = 0 },  -- Pants (Male default, will adjust logic for female)
        [5] = { drawable = 0, texture = 0 },   -- Bag
        [6] = { drawable = 34, texture = 0 },  -- Shoes
        [7] = { drawable = 0, texture = 0 },   -- Chain/Accessory
        [8] = { drawable = 15, texture = 0 },  -- Undershirt
        [9] = { drawable = 0, texture = 0 },   -- Body Armor
        [11] = { drawable = 15, texture = 0 }  -- Jacket/Torso
    },
    props = {
...
-- [NEW] Helper event to track changes from NUI
RegisterNetEvent('illenium-appearance:client:TrackChange', function(type, id, drawable, texture)
    if not changedClothes[id] then changedClothes[id] = {} end
    changedClothes[id] = {
        type = type,
        drawable = drawable,
        texture = texture
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            -- [NEW] СЪЗДАВАНЕ НА ГЕРОЙ: Пращаме всичко, за да получим item-ите
            TriggerServerEvent("ks-addons:clothes:server:EquipFullOutfit", appearance)
        ...
        changedClothes = {} -- Cleanup
    end, config)
end

function OpenShop(config, isPedMenu, shopType)
    -- [NEW] Reset tracking on shop open
    changedClothes = {}
    
    -- [NEW] Enable service mode during customization
    ...
    lib.callback("illenium-appearance:server:hasMoney", false, function(hasMoney, money)
        ...
        client.startPlayerCustomization(function(appearance)
            if appearance then
                ...
                -- [NEW] SHOP LOGIC FOR KS-ADDONS
                if shopType == "clothing" and not isPedMenu then
                    -- 1. Пращаме само промените за купуване като item-и
                    if next(changedClothes) ~= nil then
                        TriggerServerEvent("ks-addons:clothes:server:processPurchase", changedClothes)
                    end

                    -- 2. "Събличаме" героя за базата данни
                    -- Правим дълбоко копие, за да не бъгнем визуално героя в момента
                    local appearanceToSave = json.decode(json.encode(appearance))
                    local gender = Framework.GetGender()
                    local isFemale = (gender == "female")

                    -- Strip Components
                    if appearanceToSave.components then
                        for i, comp in pairs(appearanceToSave.components) do
                            local def = strippedDefaults.components[comp.component_id]
                            if def then
                                local drawable = def.drawable
                                -- Gender specific fix for pants/shoes
                                if isFemale then
                                    if comp.component_id == 4 then drawable = 14
                                    elseif comp.component_id == 6 then drawable = 35
                                    elseif comp.component_id == 8 then drawable = 14 end
                                end
                                appearanceToSave.components[i].drawable = drawable
                                appearanceToSave.components[i].texture = def.texture
                            end
                        end
                    end

                    -- Strip Props
                    if appearanceToSave.props then
                        for i, prop in pairs(appearanceToSave.props) do
                            local def = strippedDefaults.props[prop.prop_id]
                            if def then
                                appearanceToSave.props[i].drawable = def.drawable
                                appearanceToSave.props[i].texture = def.texture
                            end
                        end
                    end

                    TriggerServerEvent("illenium-appearance:server:saveAppearance", appearanceToSave)
                else
                    -- Ако е бръснар, татуист или пед меню - запазваме нормално
                    TriggerServerEvent("illenium-appearance:server:saveAppearance", appearance)
                end
            else
                lib.notify({
