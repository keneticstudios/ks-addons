...
local ClothingMap = {
    ["mask"]    = { type = "component", id = 1,  default = 0, defaultTex = 0 },
    ["hair"]    = { type = "component", id = 2,  default = 0, defaultTex = 0 },
    ["arms"]    = { type = "component", id = 3,  default = 15, defaultTex = 0 },
    ["pants"]   = { type = "component", id = 4,  default = 21, defaultTex = 0 },
    ["bag"]     = { type = "component", id = 5,  default = 0, defaultTex = 0 },
    ["shoes"]   = { type = "component", id = 6,  default = 34, defaultTex = 0 },
    ["accessory"] = { type = "component", id = 7, default = 0, defaultTex = 0 },
    ["t-shirt"] = { type = "component", id = 8,  default = 15, defaultTex = 0 },
    ["vest"]    = { type = "component", id = 9,  default = 0, defaultTex = 0 },
    ["decals"]  = { type = "component", id = 10, default = 0, defaultTex = 0 },
    ["torso2"]  = { type = "component", id = 11, default = 15, defaultTex = 0 },
    -- Props
    ["hat"]     = { type = "prop", id = 0, default = -1, defaultTex = 0 },
    ["glass"]   = { type = "prop", id = 1, default = -1, defaultTex = 0 },
    ["ear"]     = { type = "prop", id = 2, default = -1, defaultTex = 0 },
    ["watch"]   = { type = "prop", id = 6, default = -1, defaultTex = 0 },
    ["bracelet"]= { type = "prop", id = 7, default = -1, defaultTex = 0 },
}
...
local function ChangeVariation(data)
    ...
    -- Помощна функция за записване на промяната за item системата
    local function TrackItemChange(cat, valueType, value)
        if ClothingMap[cat] then -- Следим само ако е дреха/аксесоар (без лице)
            if not changedClothes[cat] then changedClothes[cat] = {} end
            changedClothes[cat][valueType] = value
            
            -- Уверяваме се, че имаме и другата стойност (ако сменихме item, записваме и текущата texture)
            if valueType == "item" and not changedClothes[cat].texture then
                changedClothes[cat].texture = skinData[cat].texture
            elseif valueType == "texture" and not changedClothes[cat].item then
                changedClothes[cat].item = skinData[cat].item
            end
        end
    end
...
local function SaveSkin()
    ...
    local isMale = (pData.charinfo.gender == 0)
    if model == GetHashKey("mp_f_freemode_01") then isMale = false end
    local isClothingShop = (inZone and zoneName == 'clothing')

    if isClothingShop then
        local purchaseData = {}
        
        for k, v in pairs(changedClothes) do
            local map = ClothingMap[k]
            if map then
                purchaseData[map.id] = {
                    drawable = v.item,
                    texture = v.texture,
        ...
        if next(purchaseData) ~= nil then
            TriggerServerEvent("ks-addons:clothes:server:processPurchase", purchaseData)
        end
        local skinToSave = json.decode(json.encode(skinData))
        
        for k, map in pairs(ClothingMap) do
            if skinToSave[k] then
                if map.type == "component" then
                    local defItem = map.default
                    if not isMale then
                        if k == "pants" then defItem = 14 
                        elseif k == "shoes" then defItem = 35 
                        elseif k == "t-shirt" then defItem = 14 
                        end
                    end
                    skinToSave[k].item = defItem
                    skinToSave[k].texture = map.defaultTex
                else

                    skinToSave[k].item = map.default
                    skinToSave[k].texture = map.defaultTex
                end
            end
        end

        local clothing = json.encode(skinToSave)
        TriggerServerEvent("qb-clothing:saveSkin", model, clothing)

    else
        local clothing = json.encode(skinData)
        TriggerServerEvent("qb-clothing:saveSkin", model, clothing)
        TriggerServerEvent("ks-addons:clothes:server:EquipFullOutfit", skinData)
    end
...
--- Deep copy of internal skinData in qb-clothing format (for ks-addons / external saves).
exports('getSkinData', function()
    return json.decode(json.encode(skinData))
end)

--- Push current ped components/props into qb-clothing's skinData (after native SetPed* from other resources).
exports('syncClothingMapFromPed', function(ped)
    ped = ped or PlayerPedId()
    for k, map in pairs(ClothingMap) do
        local slot = skinData[k]
        if slot then
            if map.type == "component" then
                slot.item = GetPedDrawableVariation(ped, map.id)
                if k == "hair" then
                    slot.texture = GetPedHairColor(ped)
                else
                    slot.texture = GetPedTextureVariation(ped, map.id)
                end
            else
                local idx = GetPedPropIndex(ped, map.id)
                slot.item = idx
                slot.texture = (idx == -1) and 0 or GetPedPropTextureIndex(ped, map.id)
            end
        end
    end
end)
