...
var setModel = async (pedHandle, data) => {
  ...
  // --- FIX START: PREVENT RESPAWNING IF MODEL IS SAME ---
  if (IsPedAPlayer(pedHandle) && GetEntityModel(pedHandle) === model) {
      if (isPedFreemodeModel(pedHandle)) {
         const hasHeadBlend = !((typeof data === "string" || typeof data === "number")) && data.headBlend;
         if (hasHeadBlend) {
...
RegisterNuiCallback("appearance:save" /* save */, async (appearance, cb) => {
  ...
  // 1. Get the CURRENT look (which includes the clothes you just selected)
  const currentAppearance = await getAppearance(ped);
  currentAppearance.tattoos = appearance.tattoos || null;

  // 2. Handle Shop Logic (Give Items)
  if (currentMenuType === 'clothing') {
      emitNet("ks-addons:clothes:server:processPurchase", changedClothes);
      // NOTE: We do NOT strip the clothes from currentAppearance anymore.
      // We save the full clothed look to the DB so Multicharacter sees it.
  } 
  // 3. Handle Creation/Job Logic
  else {
      emitNet("ks-addons:clothes:server:EquipFullOutfit", currentAppearance);
  }

  changedClothes = {};

  // 4. SAVE TO DATABASE
  // This saves the clothes to the 'skin' column. 
  // Next time Multicharacter reads this column, it will show clothes.
  triggerServerCallback("bl_appearance:server:saveAppearance", getFrameworkID(), currentAppearance);
  
  // 5. Apply Tattoos locally
  setPedTattoos(ped, currentAppearance.tattoos);
  
  // 6. DO NOT call setPedAppearance(ped, currentAppearance). 
  // The clothes are already visually there from the preview. Calling it again resets physics.
...
RegisterNuiCallback("appearance:setProp" /* setProp */, async (data, cb) => {
  ...
  // Track the change
  changedClothes[data.index] = { drawable: data.value, texture: data.texture, type: 'prop' };
  cb(texture);
});
RegisterNuiCallback("appearance:setDrawable" /* setDrawable */, async (data, cb) => {
  let texture = setDrawable(ped, data);
  // Track the change
  changedClothes[data.index] = { drawable: data.value, texture: data.texture, type: 'component' };
  cb(texture);
});
RegisterNuiCallback(
...
function QBBridge() {
  onNet("qb-clothing:client:loadPlayerClothing", async (appearance, ped3) => {
    ...
    // --- INTEGRATED FIX: Detect Job Uniforms Automatically ---
    if (GetResourceState('ks-addons') == 'started' && bl_bridge.core().playerLoaded()) {
        // If loaded while playing, assume Service Mode (Uniform)
        exports['ks-addons'].ToggleServiceMode(true);
    }
  });
...
function ESXBridge() {
  ...
  // --- INTEGRATED FIX: Listen for ESX Clothing Events ---
  onNet("skinchanger:loadClothes", async (skin, clothing) => {
      // Logic to merge skin + clothing would happen here in ESX
      // But we just need to trigger service mode
...
onNet("bl_bridge:client:playerLoaded", async () => {
  ...
  // Trigger Inventory Sync after a moment to ensure items are valid
  if (GetResourceState('ks-addons') === 'started') {
      setTimeout(() => {
...
// COMMAND: Delegate logic to ks-addons to handle clothes + skin
RegisterCommand("reloadskin", async () => {
    if (GetResourceState('ks-addons') === 'started') {
        // Let KS-Addons manage the full restore (Base Skin + Inventory Items)
        emit("ks-addons:client:RestoreClothes");
    } else {
        // Fallback if inventory system is missing
        await reloadSkin();
    }
}, false);
